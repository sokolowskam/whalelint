...
func TestValidateRun015(t *testing.T) {
	t.Parallel()
	...
	testCases := []struct {
		...
		{commandStr: "cat /priv_key < echo \" -----BEGIN OPENSSH PRIVATE KEY----- \"", isViolation: true},
	}
	...
	for _, testCase := range testCases {
		...
		t.Run(testCase.commandStr, func(t *testing.T) {
			...
			assert.Equal(t, testCase.isViolation, RuleSet.ValidateRun015(runCommandWithoutSudo).IsViolated())
		})
	}
}
