...
func TestValidateRun012(t *testing.T) {
	t.Parallel()
	...
	testCases := []struct {
		...
		{commandStr: "echo '/dummy/line' >> /etc/sudoers", isViolation: true},
	}
	...
	for _, testCase := range testCases {
		...
		t.Run(testCase.commandStr, func(t *testing.T) {
			...
			assert.Equal(t, testCase.isViolation, RuleSet.ValidateRun012(runCommandWithoutSudo).IsViolated())
		})
	}
}
